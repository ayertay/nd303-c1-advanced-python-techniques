...
class VelocityFilter(AttributeFilter):
    """NEO Velocity filter class."""

    @classmethod
    def get(cls, approach):
        """Return NEO's appoach velocity."""
        return approach.velocity


class DateFilter(AttributeFilter):
    """NEO Date filter class."""

    @classmethod
    def get(cls, approach):
        """Return date of the appoach."""
        return approach.time.date()


class DiameterFilter(AttributeFilter):
    """NEO Diameter filter class."""
...
class HazardousFilter(AttributeFilter):
    """NEO Hazardous filter class."""

    @classmethod
    def get(cls, approach):
        """Return whether NEO is hazardous or not."""
...
def create_filters(
    ...
    data = []
    if date:
        data.append(DateFilter(operator.eq, date))
    if start_date:
        data.append(DateFilter(operator.ge, start_date))
    if end_date:
        data.append(DateFilter(operator.le, end_date))
    if distance_min is not None:
        data.append(DistanceFilter(operator.ge, distance_min))
    if distance_max:
        data.append(DistanceFilter(operator.le, distance_max))
    if velocity_min is not None:
        data.append(VelocityFilter(operator.ge, velocity_min))
    if velocity_max is not None:
        data.append(VelocityFilter(operator.le, velocity_max))
    if diameter_min is not None:
        data.append(DiameterFilter(operator.ge, diameter_min))
    if diameter_max is not None:
        data.append(DiameterFilter(operator.le, diameter_max))
    if hazardous is not None:
        data.append(HazardousFilter(operator.eq, hazardous))
